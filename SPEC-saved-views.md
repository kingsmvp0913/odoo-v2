...
> ## ✅ 執行狀態：2026-08-08 已完成（commit `56810a0`，階段 1＋2 皆做）
>
> 錨點複驗：本文引用的位置全部準確。§2.3 的專用端點約束已照做（`PUT /api/settings/views`），
> 並以「存 view 後 theme 與 odoo_url 必須原樣還在」當迴歸測試——已用故意改成整包覆寫驗證該測試會紅。
>
> **與本文的一處差異**：§3 階段 1 要求「新增一顆重設篩選按鈕」——**它已經存在**。`TaskList.js` 的
> `clearFilters`（按鈕在篩選列末尾）清的正是 `activeFilterCount` 計算的那 6 項，完全對應；它刻意
> 不動 `filter`／`sort`／`showAllUsers`（那三個是「看哪一類、怎麼排、看誰的」，不算篩選條件）。
> 故未新增第二顆——再加一顆只是重複，且照本文語意寫出來的版本會連 tab 與排序一起重設。
>
> 前端無自動化測試，§4.1 的人工測試清單仍待執行（含深色模式）。

...
`proposal` 不是「寫給人看的建議」。核准（含預設核准）之後，當晚就會被拿去自動實作、跑測試、合併——**沒有人會先看過**。這份判準因此不是建議語氣，是產出前的硬性關卡：過不了下面的證據門檻就不准升格成 `proposal`，只能留在 `signal`。
...
**硬性規則，不是建議**：每個 `proposal` 必須指名 `target_metric`（動哪一個指標）、`metric_baseline`（現值多少）、預期往哪個方向。三者缺一即不成立——**不得升格成 `proposal`，改寫成 `kind='signal'`**（候選訊號，存著等下一輪累積證據，不進自動實作通道）。「有幫助」「看起來該修」都不是指標，不能填進這三欄充數。

不確定、講不出這三欄、或只有單一任務證據的，一律 `kind='signal'`。**寧可少提，不可硬升格**——`proposal` 當晚就會被自動實作合併，沒有覆核，錯的提案就是錯的程式碼上線。
...
- **提案講不出要動哪個指標** → 「有幫助」不是驗收條件，這條不是 `proposal`，是 `signal`
- **證據不足卻硬升格成 `proposal`** → 當晚會被自動實作合併，沒人先看過，錯了就是錯的碼上線

...
你在無人值守的 pipeline 中執行：沒有人會即時回你話，**但你有發問管道**。查得到證據、規格判得準的，自己判完收尾，不要問；**規格模糊／矛盾／你與實作對意圖認知不同**的，走 `spec_questions`（分流判準見下方）——那會停下來問使用者，而且要**一次問完**，不要分批。
不得在 <result> 契約之外輸出自然語言問句或等待回覆（那會讓本關解析失敗、整輪報廢）；有疑問就填進契約裡的欄位，不是寫在契約外面。
...
3. 檢查 Odoo 規範違反：
   ...
   - try/except 涵蓋範圍與錯誤訊息歸屬（**本次 diff 若有新增或修改 `try:` 區塊才查**；這類缺陷「程式跑得起來」，deploy 必定綠燈，diff 上 try/except 形狀也很正常，只有踩到錯誤路徑的使用者才會遇到）。對 diff 內每個 try 區塊逐項對照：
     ① **邊界**：try 內是否混進了與該 except 語意無關的行——前置準備（取參數、查 record、組 payload）與後續動作（寫回欄位、回傳、發通知）都應該在 try 外；同一個 try 內混著兩種失敗語意（如「連外部系統」＋「寫 Odoo 欄位」）也算，該拆成兩個 try。
     ② **訊息歸屬**：except 產生的訊息文字（`UserError`／`_logger`）講的那個操作，是否真的落在 try 區塊內——對不上就是張冠李戴，使用者看到的錯誤指向錯的操作，無法定位真因。
     ③ **吞成成功**：except 是否既不重拋、也不把失敗反映到回傳值／狀態就往下走（只 `pass`、只 `_logger.warning`）——子操作的失敗會被上層 except 吞掉，流程照常走完並回報成功，使用者以為做好了其實沒做。
...
【小修正規格（人工審核退回後追加）】
{{tweak_specs}}
這是使用者在最終人工審核看到成品後退回、由分診員寫下的**追加規格**。主規格（上面那份）不會為它改寫，
所以**兩份要合起來看**：與主規格牴觸時以本區為準（它比主規格晚、而且是使用者看過實物之後說的）。
本區寫到的行為即使主規格一個字都沒提，也**不算「規格以外的改動」**——照它做的實作是對的，不得判 fail。

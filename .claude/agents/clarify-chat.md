...
【設計稿：對話或規格裡出現 figma.com 連結時，這件事你查得到】
```bash
curl -H "X-AIDEV-AI-TOKEN: $AIDEV_AI_TOKEN" "$AIDEV_AI_BASE/ai/figma?url=<把整條 figma 連結做 URL 編碼>"
```
回的是扁平節點清單（`type`／`text`／`x`／`y`／`w`／`h`，以及有才給的 `fill`／`font`／`radius`／`name`）。
- **不要用 WebFetch**：Figma 頁面是 JS 渲染的，只拿得到空殼，然後你會把「我抓不到」當成「系統開不了 Figma」。
- 規格裡若寫著「讀不到設計稿」，那是**分析關當時**的結論，不是現在的事實——使用者追問「設計稿就在連結裡」「現在讀得到了嗎」時，**自己打一次再回答**，不要把舊結論原地複述一遍。
- 只有回 503（平台未設 token）或 403（帳號看不到該檔）才是真的讀不到，此時明講是哪一種。
- 讀到之後若某題因此不必再問了，那就是「談出結論」→ 走 `revise` 把題目重產，別讓使用者去答一個你已經自己查到答案的題目。

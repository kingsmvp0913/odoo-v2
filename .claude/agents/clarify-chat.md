...
基本處置見上方【figma】那段（使用者追問「設計稿就在連結裡」「現在讀得到了嗎」時同樣適用）。
**本關的出口**：需要的細節就當成題目問出來，走 `revise` 補進題目清單，別讓它停在「我看不到設計稿」就沒有下文。
...
【題目撰寫契約——revise 時必須遵守】
**格式一律照上方【題目撰寫契約】**，逐條適用，此處不重複。本關（就地改寫既有題目）另有四條：
- 使用者在對話中已經回答過的題目，直接把答案填進 `answer` 欄，不要再問一次。
- 使用者已明確表示不需要的功能，該題直接刪除。
- 原本就有 `recommended` 的題目除非結論推翻了它，否則原樣保留——
  你這一輪重產的題目會整份取代畫面，漏抄等於把建議刪掉。
- **對話中浮現的新分歧要補成題目**，不要因為「這場對話是來收斂的」就把它吞掉——哪三種最容易漏，
  見上方【這三種一律要問】；照該段判準，說得出有依據的預設就在 `REPLY` 告知你的決定，不必補成題目。

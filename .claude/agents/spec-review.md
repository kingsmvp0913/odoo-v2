...
- **引用錯誤訊息／例外文字／程式碼片段時，整個字串一律用單引號包住**（字串內原有的單引號寫成兩個 `''`）：
  ...
  `unsupported operand type(s) for -: 'datetime.date'` 這種原文會讓整份規格解析失敗、整輪報廢。
  多行 `|` 區塊（如 `summary:`）內不受此限。寧可多加引號——多加無害，少加一次就是整張任務卡住等人工。
- 拿不準使用者到底要不要改、或要改成什麼 → 不要硬改，走 `answer` 反問。

【什麼情況一定要反問——不是「可以問」】
哪三種見上方【這三種一律要問】。**本關的出口**：命中任一種、且你**說不出有依據的預設**時，走 `answer`
問清楚再改，不要硬改進規格；說得出依據的就照依據改，在 `REPLY` 用一句話說明你依據什麼這樣決定。

...
- **判準只有一個：他讀不讀得懂。不是字數。** 需求大就寫長，該解釋的一句都不能省——
  解釋術語本來就會讓字變多，那是對的方向，不要為了短而砍掉解釋。

- **第一次出現的專有名詞要補一句白話**——規則見上方【說人話守則】L5，本欄逐字適用，是這裡最重要的一條。
  下面的輸出範例就是照它寫的，照抄那個樣子。

- **建議的骨架**（不合用可調整，但這四段的資訊都要有）：
    【哪裡壞了】現在是什麼狀況、他會遇到什麼問題
    【要改成什麼】改完之後在哪個畫面、按什麼、會看到什麼結果
    【不會動到的】哪些現有功能維持原樣（讓他放心）
    【要你確認】這張任務的關鍵取捨，請他過目
  **小標用【】，不要用 markdown 的 `**`**——這欄在畫面上是純文字，星號會原樣露出來。
  一句話講一件事，一行一句，不要擠成一大段。
...
summary: |
  【哪裡壞了】
  「報價單」（給客戶報價用的單子）上的備註欄現在是唯讀的，看得到但打不了字。
  業務想在上面補一句交期說明，只能改到別的地方去，客戶收到的報價單上看不到。

  【要改成什麼】
  報價單的「備註」欄改成可以直接打字，可以換行。
  存檔後重新打開，打的字還在，也會印在報價單上。

  【不會動到的】
  這個備註不會回寫到來源工單，工單那邊維持原樣。

  【要你確認】
  備註只有「銷售 / 使用者」群組的人看得到、改得動，這樣可以嗎？
findings: |

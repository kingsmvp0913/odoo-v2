...
3. **自己查資料的能力**：載入 platformDB skill 後可以對平台資料庫下 SQL（唯讀，只能 SELECT）。表：`tasks`、`token_usage`、`task_events`、`task_rejections`／`rejection_items`、`health_check_findings`、`project_chats`／`project_chat_messages`（排障對話的原文）。
...
### `chat_quality`（排障對話的品質）怎麼讀

起手包的其他區塊講的都是「花多少錢、跑幾次」，只有這一塊在講 chat **答得好不好**。這一塊上線前，使用者實際感受到的「鬼打牆、廢話太多」健檢一律報平安（2026-09-05 回報）。

- `verbosity_ratio_p50`／`_max`：AI 回覆長度 ÷ 使用者提問長度。上線當下的基線是 p50 **15.1**、max **44.4**。這個數字要往下走。
- `self_correcting_chats`／`self_correct_turns`：AI 在回覆裡承認自己上一輪判斷錯的場次與輪次。基線是 34 場裡有 **10 場**、共 **18 輪**。它代表「使用者用領域知識糾正、AI 才修」——等於使用者在幫 agent 除錯，是這條通道最真實的品質訊號。
- `worst`：最該去看的三場，附 `chat_id`。

⚠ **`self_correct` 是關鍵字比對，不是語意判斷**：換個說法就抓不到（會漏），正常的「你說的對」也會被算進去（會誤判）。所以**它只能拿來決定「去看哪一場」，不能當成提案的唯一證據**——要用 `chat_id` 去 `project_chat_messages` 讀那場對話的原文、確認真的在鬼打牆，才算數。拿數字直接生提案，判準那條「證據強度決定能動哪一層」就沒過。

⚠ 這一塊回 `{ chats: 0, note: ... }` 時代表**窗內沒有來回兩輪以上的對話**，不是「都很好」。照實寫零樣本，不得據此宣稱 chat 正常。

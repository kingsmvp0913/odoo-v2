...
**沒有人在讀你的回覆，也沒有人會補資料給你。** 你是背景批次，輸出直接進統計表。
所以資訊不足時不要反問、不要說明你為什麼做不到——那些話沒有收件人，而整筆會因為
少了 <result> 被判成失敗丟掉（實測：退回原因寫「請按照附件圖片修正」時，你回了一段
向使用者要圖的文字，那一筆就此消失在統計外）。

看不懂、資訊不足、原因指向你看不到的附件時，照這樣輸出——把原文整句照抄進 description：
<result>
[{"description":"有三個不符合分析書的地方，請按照附件圖片修正（原因指向附件，無法拆項）","category":"其他"}]
</result>

原因文字完全沒有可辨識的錯誤項時（例如只是一句測試字串），回空陣列：
<result>
[]
</result>

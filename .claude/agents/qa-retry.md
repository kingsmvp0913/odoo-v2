...
### 失敗回報要求
本對話上一輪 prompt 的【失敗回報要求】本輪一字未改，逐條照舊適用，此處不重複——判定前重讀那一段，
特別是這三條：**一次窮盡列出**（不分批）、**規格與核心原始碼牴觸時走 `spec_questions` 而非判 fail**
（照原始碼寫的實作是對的；注意同一屬性名在不同層各有解析器，拿錯那層的碼會結論相反）、
**每題附建議答案與一句依據**（純屬「使用者要什麼」的定義題除外，只問不建議）。
分流判準照舊：實作寫錯→`issues`；規格模糊／矛盾／無法裁決→`spec_questions`；**判不準時偏向 `issues`**
（不要濫用 spec_questions 製造不必要的停等），但「規格與原始碼牴觸」不算判不準，一律走 `spec_questions`。

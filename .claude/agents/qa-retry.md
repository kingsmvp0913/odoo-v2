...
【小修正規格（人工審核退回後追加）】
{{tweak_specs}}
本對話裡的分析規格是上一輪帶進來的，**不含本區**——這是本輪唯一會變動的規格來源，所以照這裡為準。
它是使用者在最終人工審核看到成品後退回、由分診員寫下的追加規格；主規格不會為它改寫，兩份合起來看，
牴觸時以本區為準。本區寫到的行為即使本對話先前談的規格一個字都沒提，也**不算「規格以外的改動」**，
不得因此判 fail——否則實作照使用者說的做、你照舊規格退，兩邊各自都對，碼怎麼改都過不了。

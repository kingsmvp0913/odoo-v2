...
你的工作目錄裡有一支 `odooDev` skill。**要判斷「這是 bug 還是原生行為」、或要自己造測試資料之前，先載入 odooDev 判準。** 文件在 `.agents/skills/odooDev/SKILL.md`（Claude Code 為 `.claude/skills/odooDev/SKILL.md`）；它不重複你已經有的硬規則，只放需要時才查的東西；載不到就照常進行，不要為此停下來。

...
async function syncUser(userId) {
  ...
  // 來源結案不再連動封存平台任務：sync 只負責把單「拉進來／更新」，封存改為 30 天自動（cron
  // autoArchiveDone）或手動（tasks-routes /archive）。來源用 fold／state 判結案常誤傷（如 Odoo
  // 折疊欄位不必然＝結案、eService 一離開 open 就被掃到），使既有任務莫名秒封存，故移除。
  return { odoo, service };
}

...
test('triageTask → analysis_running when triage says so', async () => {
  mockCallClaude.mockResolvedValue({ text: JSON.stringify({ outcome: 'analysis_running', content: '需求清晰，開始分析' }), usage: null, durationMs: null });
...
test('triageTask → answered when triage says so', async () => {
  mockCallClaude.mockResolvedValue({ text: JSON.stringify({ outcome: 'answered', content: '這個問題的答案是...' }), usage: null, durationMs: null });
...
test('triageTask → triage_blocked when triage says so', async () => {
  mockCallClaude.mockResolvedValue({ text: JSON.stringify({ outcome: 'triage_blocked', content: '無法透過標準方式實現' }), usage: null, durationMs: null });
...
test('triageTask → confirm_pending with clarification questions', async () => {
  mockCallClaude.mockResolvedValue({ text: JSON.stringify({
...
test('triageTask → triage_blocked when claude returns invalid JSON', async () => {
  mockCallClaude.mockResolvedValue({ text: 'this is not valid JSON', usage: null, durationMs: null });
...
test('triageNewTasks processes all new tasks for user', async () => {
  mockCallClaude.mockResolvedValue({ text: JSON.stringify({ outcome: 'analysis_running', content: 'ok' }), usage: null, durationMs: null });

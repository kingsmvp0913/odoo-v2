...
describe('updateAgent', () => {
  ...
  test('改 prompt 會寫入新 body（保留既有 placeholder）', () => {
    ...
    const p = '新的提示詞 {{project_name}} {{wiki}} {{data_source_hint}} {{chat_files_dir}} {{history}} {{user_message}}';
    const updated = L.updateAgent('chat', { prompt: p });
    expect(updated.body.trim()).toBe(p);
    expect(updated.render({ project_name: 'P', wiki: 'W', data_source_hint: '', chat_files_dir: '/o', history: 'H', user_message: 'X' })).toContain('X');
  });

// 意圖：釘住健檢頁上「要不要改善」那一欄的誠實度，以及「處置一律不在健檢頁」這條界線。
//
// ⚠ 這支原本還釘著「HC_STATUS（前端裁決鈕送出的值）⊆ FINDING_STATUS（後端 PATCH 白名單）」。
// HC_STATUS 住在 js/views/AdminHealthCheck.js，而那整個舊版前端已於 2026-09-22 退役刪除；
// ui-next 的健檢頁是唯讀的（見下面那條守衛），畫面上不再有任何送出 finding 狀態的按鈕。
// 那條跨前後端的守衛因此**沒有對象**了——是刪掉，不是改寬。
// 後端 admin-routes.js 的 FINDING_STATUS 白名單仍在（API 還收得到 PATCH），只是沒有 UI 會送。
const fs = require('fs');

...
// （2026-09-22 舊版前端退役：對象改成現在唯一在跑的 ui-next 版。）
test('AdminAgents.js 語法有效，能註冊 Agent 管理 View', () => {
  const file = path.join(__dirname, '../../public/js/ui-next/pages/AdminAgents.js');
...
test('Agent 管理右欄要跟著捲動留在畫面上：sticky 與它的捲動祖線解法必須成對存在', () => {
  ...
  // 頁面仍沿用 app.css 的 .aa-editor，要掛得到 class 才受規則影響。

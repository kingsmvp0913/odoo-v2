...
<button class="ui-next-primary" @click="save" :disabled="saving">{{ saving?'儲存中…':'儲存' }}</button>
  ...
  window.UiNextProjectListView = Vue.defineComponent({
    ...
    methods: {
      ...
      unread(id) { return window.UnreadStore.byProject[String(id)] || 0; }, go(id) { this.$router.push(`/projects/${id}`); }, goWiki(id) { this.$router.push(`/projects/${id}/wiki`); }, goChat(id) { this.$router.push(`/projects/${id}/chat`); }, goDb(id) { this.$router.push(`/projects/${id}/db`); }, goDeploySop(id) { this.$router.push(`/projects/${id}/deploy-sop`); }, isAdmin() { return window.UserStore.role === "admin"; },
...
<button class="ui-next-primary" data-tour="proj-add" @click="showAddForm=!showAddForm">{{ showAddForm?'取消':'新增專案' }}</button>
</header>
<section v-if="showAddForm" class="ui-next-project-create" data-tour="proj-form">
<label>專案名稱<input v-model="newProject.name" placeholder="例：my-odoo">
</label>
<label>Odoo 版本<input v-model="newProject.odoo_version" placeholder="例：17.0">
</label>
...
<!-- folder_name 的說明不能省：這個值同時決定測試容器名、環境目錄與資料庫名，
     填中文會被清成空字串，所有中文專案共用同一個容器而互砍（已實際發生過）。 -->
<label class="ui-next-create-wide"><span>英文資料夾名稱 <span class="ui-next-required">*</span> <small>測試環境的容器、目錄與資料庫都用它命名</small></span><input v-model="newProject.folder_name" placeholder="例：hong-jiu（限英文、數字、底線、連字號）">
</label>
<label class="ui-next-create-wide">說明（選填）<textarea v-model="newProject.description" placeholder="專案描述…">
</textarea>
</label>
<div class="ui-next-create-actions">
...
<header>
<button v-if="project.id!=='demo'" @click="toggleFavorite(project)" :class="{active:project.is_favorite}" :aria-label="project.is_favorite?'取消我的最愛':'加入我的最愛'"><ui-next-icon name="star"/></button>
...
<button title="把 push 接成自動部署的步驟說明" @click="goDeploySop(project.id)">自動部署 SOP</button>
<button v-if="!project.has_wiki" @click="initWiki(project.id)">初始化 Wiki</button>
<button @click="go(project.id)">管理</button>
<button v-if="isAdmin()" style="color:var(--danger)" @click="remove(project)">刪除專案</button>
</footer>
</article>
<!-- 空狀態要分流：新帳號一個專案都沒有時看到「找不到符合的專案」會以為是篩選壞了。 -->
<p v-if="!filteredProjects.length" class="ui-next-empty-state">{{ search ? '沒有符合的專案。' : '尚無專案，點擊「新增專案」開始建立。' }}</p>
</div>

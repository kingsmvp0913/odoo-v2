(function () {
  ...
  const UiNextIcon = Vue.defineComponent({
    ...
    template: `<svg class="ui-next-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.9" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">
      ...
      <path v-else-if="name==='book'" d="M4 5.5A2.5 2.5 0 0 1 6.5 3H20v16H6.5A2.5 2.5 0 0 0 4 21.5zM4 5.5v16"/><path v-else-if="name==='graduation'" d="m2 8.5 10-4.5 10 4.5-10 4.5zM6 10.7V16c0 1.7 2.7 3 6 3s6-1.3 6-3v-5.3"/><path v-else-if="name==='chart'" d="M5 20v-8m7 8V4m7 16v-5"/>
      <path v-else-if="name==='paperclip'" d="m21 11.5-8.7 8.7a5 5 0 0 1-7.1-7.1l8.8-8.8a3.5 3.5 0 0 1 5 5l-8.8 8.8a2 2 0 0 1-2.8-2.8l8.1-8.1"/><path v-else-if="name==='code'" d="m8 6-6 6 6 6m8-12 6 6-6 6"/><path v-else-if="name==='pause'" d="M9 5v14M15 5v14"/><path v-else-if="name==='play'" d="M8 5.5v13l11-6.5z" fill="currentColor"/><path v-else-if="name==='terminal'" d="m5 7 5 5-5 5m8 1h6"/><path v-else-if="name==='archive'" d="M3 6h18v4H3zm2 4v9h14v-9M9.5 14h5"/>
  ...
  window.UiNextQuestionView = Vue.defineComponent({
    ...
    template: `
      <section class="ui-next-question" :class="{'is-launching':launching}">
        <div ref="questionInner" data-tour="home-question" class="ui-next-question-inner">
  ...
  window.UiNextApp = Vue.defineComponent({
    ...
    computed: {
      ...
      // 所以上完一課或角色載入後這裡會自己重算。tour.js 沒載入時退成 0（badge 不顯示）。
      tourRemaining() {
        return window.TourManager ? window.TourManager.remainingCount() : 0;
      },
      ...
      sidebarProjects() {
        ...
        // 「最近有活動」＝15 天內有對話訊息或任務更新。這兩份資料側欄本來就都載了
        // （sidebarChatProjects 帶 last_message_at、sidebarTasks 帶 project_id＋updated_at），
        // 所以不必為此再多打一支 API。封存的任務不算活動：GET /api/tasks 預設只回 is_hidden=false。
        const since = Date.now() - 15 * 24 * 60 * 60 * 1000;
        const addIfRecent = (projectId, at) => {
          // 取反判斷：at 缺值或格式壞掉時 Date.parse 回 NaN，任何比較都是 false，直接不算最近。
          if (!projectId || !(Date.parse(at) >= since)) return;
          const project = projectById.get(String(projectId));
          if (project) selected.set(String(project.id), project);
        };
        this.sidebarChatProjects.forEach((item) => addIfRecent(item.project_id, item.last_message_at));
        this.sidebarTasks.forEach((task) => addIfRecent(task.project_id, task.updated_at));
    ...
    template: `
      ...
      <div v-else class="ui-next-shell" :class="{ 'has-ribbon': !!ribbon }" data-ui="next">
        ...
        <aside ref="mobileSidebar" class="ui-next-sidebar" :class="{ 'is-mobile-open': mobileSidebarOpen }" :role="mobileSidebarOpen ? 'dialog' : null" :aria-modal="mobileSidebarOpen ? 'true' : null" :aria-label="mobileSidebarOpen ? '主選單' : null" :tabindex="mobileSidebarOpen ? '-1' : null" @keydown="trapSidebarFocus">
          ...
          <button data-tour="nav-new" class="ui-next-new" @click="go('/')"><ui-next-icon name="plus"/>新對話</button>
          <button data-tour="nav-search" class="ui-next-search" title="搜尋（⌘ K）" @click="showSearch($event)"><ui-next-icon name="search"/>搜尋</button>

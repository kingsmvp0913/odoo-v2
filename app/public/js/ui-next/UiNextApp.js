    ...
    template: `
      ...
      <div v-else class="ui-next-shell">
        <aside class="ui-next-sidebar">
          ...
          <button class="ui-next-nav" @click="go('/tasks')"><ui-next-icon name="tasks"/>任務列表 <span v-if="needsActionCount">{{ needsActionCount }}</span></button>
          <button class="ui-next-nav" @click="go('/projects')"><ui-next-icon name="project"/>專案 <span v-if="projectUnreadTotal">{{ projectUnreadTotal }}</span></button>
          <button class="ui-next-nav" @click="go('/admin/pipelines')"><ui-next-icon name="flow"/>進行中 Pipeline</button>
          <button class="ui-next-nav" @click="go('/token-report')" v-if="isAdmin"><ui-next-icon name="chart"/>用量報表</button>

(function () {
  ...
  // - srcdoc 裡的相對網址是用平台頁面的網址解析：規格頁選單 <a href="#rollout"> 會把 iframe 導到「平台/#rollout」，
  //   載入平台首頁又讀不到 token，畫面一片空白 ⇒ 補 <base href="about:srcdoc">，讓 #連結留在 iframe 自己裡面
  ...
  window.UiNextSaasSpecsView = Vue.defineComponent({
    ...
    computed: {
      frameHtml() {
        return this.html.replace(/^(\s*<!doctype[^>]*>)?/i, (doctype) => doctype + FRAME_BASE);
      },
    },
    async created() {
      try {

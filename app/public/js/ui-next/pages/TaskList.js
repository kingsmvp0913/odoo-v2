(function () {
  ...
  window.UiNextTaskListView = Vue.defineComponent({
    ...
    beforeUnmount() { SocketManager.setRefreshCallback(null); this.newPreviews.forEach((url) => { if (url) URL.revokeObjectURL(url); }); },
    methods: {
      ...
      openAdd(event) { this.newTask = { title: "", original_text: "", project_id: "" }; this.newFiles = []; this.syncAddPreviews(); this.addError = ""; this.addTrigger = event?.currentTarget || null; this.showAdd = true; this.$nextTick(() => this.$refs.newTaskTitle?.focus()); }, closeAdd() { this.showAdd = false; this.newFiles = []; this.syncAddPreviews(); this.$nextTick(() => this.addTrigger?.focus()); }, onAddFilesSelected(event) { const files = Array.from(event.target.files || []); this.addError = files.length > 5 ? "最多上傳 5 個附件，請重新選擇。" : ""; this.newFiles = files.slice(0, 5); event.target.value = ""; this.syncAddPreviews(); }, removeAddFile(index) { this.newFiles.splice(index, 1); this.syncAddPreviews(); },
      // 截圖直接貼上：描述問題最常走這條，只能「點此選擇檔案」等於逼人先存檔再翻資料夾。
      // 限制與對話那邊同一組（單檔 10MB、最多 5 個）；貼上來的只會是圖，選檔那條路仍不限型別。
      onAddPaste(event) {
        const files = Array.from((event.clipboardData || {}).files || []).filter((file) => file.type.startsWith("image/"));
        if (!files.length) return;
        event.preventDefault();
        const before = this.newFiles.length;
        files.forEach((file) => { if (file.size <= 10 * 1024 * 1024 && this.newFiles.length < 5) this.newFiles.push(file); });
        this.addError = this.newFiles.length - before < files.length ? "附件限單檔 10MB、最多 5 個，部分圖片未加入。" : "";
        this.syncAddPreviews();
      },
      // 每次變動整組重建：分兩個陣列各自 push／splice，刪掉中間一張就會錯位，縮圖對不上檔案。
      // 非圖片留空字串，版面退回原本的迴紋針圖示。
      syncAddPreviews() {
        this.newPreviews.forEach((url) => { if (url) URL.revokeObjectURL(url); });
        this.newPreviews = this.newFiles.map((file) => (file.type.startsWith("image/") ? URL.createObjectURL(file) : ""));
      }, trapAddFocus(event) { if (event.key === "Escape") return this.closeAdd(); if (event.key !== "Tab") return; const items = Array.from(this.$refs.taskCreateModal?.querySelectorAll("button:not([disabled]), input:not([disabled]), textarea:not([disabled]), select:not([disabled])") || []); if (!items.length) return; const first = items[0], last = items[items.length - 1]; if (event.shiftKey && document.activeElement === first) { event.preventDefault(); last.focus(); } else if (!event.shiftKey && document.activeElement === last) { event.preventDefault(); first.focus(); } }, async submitAdd() { if (!this.newTask.project_id || !this.newTask.title.trim() || !this.newTask.original_text.trim()) { this.addError = "請完整填寫專案、標題與內容。"; return; } this.adding = true; this.addError = ""; try { const form = new FormData(); form.append("title", this.newTask.title.trim()); form.append("original_text", this.newTask.original_text); form.append("project_id", this.newTask.project_id); this.newFiles.forEach((file) => form.append("files", file)); await Api.postForm("tasks", form); this.showAdd = false; this.newFiles = []; this.syncAddPreviews(); this.filter = "all"; showToast("已新增任務", "success"); } catch (error) { this.addError = error.message || "新增任務失敗"; showToast(this.addError, "error", 0); } finally { this.adding = false; } },
...
<span v-for="(file,index) in newFiles" :key="file.name+file.size+index" class="ui-next-file-preview"><img v-if="newPreviews[index]" class="ui-next-thumb" :src="newPreviews[index]" :alt="file.name" title="點擊放大" @click="previewImage({src:newPreviews[index],alt:file.name})"><ui-next-icon v-else name="paperclip"/><em>{{ file.name }}</em><button type="button" :aria-label="'移除附件：'+file.name" @click="removeAddFile(index)"><ui-next-icon name="close"/></button></span>
</div>
</div>

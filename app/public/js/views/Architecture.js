// 系統地景圖：把「這套體系有哪些地方、彼此怎麼連」畫成直立分區圖。
//
// 與 views/PipelineFlow.js 是姊妹頁，版面語法刻意相同（分區＝直欄、由上而下、曼哈頓連線），
// 兩頁一起看時不必重新學一次讀圖規則。但**路由邏輯刻意寫得比流程圖簡單**：流程圖有匯流排、
// 退回線、四種讓道規則，那是因為它的連線會回頭；地景圖的連線幾乎都是單向且不回頭，
// 照抄那一套只會多出用不到的分支。要加規則前先確認新連線真的不是下面四種形狀之一。
//
// ── 連線的四種形狀（只有這四種，新增連線請先確認落在哪一種）──────
// 1. 同列不同區 → 一條水平直線（最好讀，spec 的 step 就是為了湊出這種）
// 2. 同區相鄰列 → 一條垂直直線
// 3. 同區跨列   → 從側邊繞出去，走該區外側的走廊
// 4. 其他       → 垂直出→列間走廊橫走→垂直進（跨幾區都一樣）
//
// ── 三種重疊都會自動讓道（軌道分配）────────────────────────
// 同一條走廊有多條線時，後來的往外讓一軌。三處各有自己的軌道計數：
//   列間走廊（水平線的 y）、區外走廊（垂直線的 x）、方塊上下緣的出入口（x）。
// 讓道量刻意大於線寬與字高：擠在一起的兩條線在畫面上就是一條粗線，而那不會有任何錯誤訊息。
//
// ── 改完怎麼驗 ──────────────────────────────────────────
// 與流程圖同一套：開 DevTools Console 貼 views/PipelineFlow.js 檔頭那段掃描器（它讀的是
// `.page-body svg`，兩頁通用），期望 重疊／穿過方塊／字貼框 恆為 0。量測前先清 hover：
//   [...S.querySelectorAll('g')].forEach(g=>g.dispatchEvent(new MouseEvent('mouseleave')))
// 這頁的交叉**應該**趨近 0——地景圖沒有回頭路。出現交叉時先懷疑某個系統的 step 排錯了列，
// 或繞行方向挑錯邊（見形狀 3），改 spec 的 step 通常比在這裡加讓道規則有效。
//
// 交叉目前是 0，而且**是靠匯流排換來的**：程式碼倉庫那三格各自拉線時，六條線在同一條走廊裡
// 上下穿插，其中「核心→測試區」與「GitHub→AI 代理」在對面那一欄上下對調位置，先後順序必須
// 交換一次——那個交叉試過四種排法都消不掉（換軌道順序、換出入口順序、對調核心與企業版的列、
// 把 GitHub 排到核心上面），只會搬家或改成穿過方塊。合成一條主幹、分兩個出口之後它自然消失。
// 所以看到交叉先想「這幾條線是不是在講同一件事」，而不是先去調讓道規則。
//
// 內容（有哪些系統、怎麼連）全在 js/architecture-spec.js，這個檔只負責「怎麼畫」。
...
window.ArchitectureView = Vue.defineComponent({

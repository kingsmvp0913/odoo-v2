<!-- converted from 研發報價單.xlsx -->

## Sheet: 報價單
| 產品名稱            |  | {{ docs.product_name or '' }} |  |  |  |  |  |  |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 廠別  |  | {{ docs.partner_id.name or '' }} | 客戶名字 | {{ docs.contact_id.name if docs.contact_id else '' }} | 產品料號  | {{ docs.product_tmpl_id.default_code if docs.product_tmpl_id else '' }} | 單據日期 | {{ docs.maintenance_date.strftime('%Y/%m/%d') if docs.maintenance_date else '' }} |

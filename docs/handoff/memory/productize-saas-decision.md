...
description: 平台產品化（SaaS 多租戶）程式部分 2026-09-24 結案；規格書與規格頁已整個刪除，細節只剩 git 歷史
metadata: 
  node_type: memory
  type: project
...
## ⚠ 2026-09-24：程式部分結案，規格書已刪

產品化的程式工作做完了，使用者裁決把**規格頁與 9 份規格書整個拿掉**（commit `8349c0d0`）。
下面這整份記憶寫於 09-17，講的是當時的擋路現況與岔路，**多數已經不成立**，留著是因為它
記錄了幾個裁決的理由。要查規格原文：`git show 250fb83a:docs/superpowers/specs/<檔名>`。

收尾那一輪實際做完的（都已 push）：
- 拿掉「不進容器」的舊路徑（`8aa373ab`）——連帶結清 canRun 的結構性缺口。代價是 10 支
  測試套件要重寫，因為它們原本靠「模組預設 off」拿到同步 spawn。
- 四身分實機驗收（Playwright，80 張截圖）找到三個壞掉的入口（`08c1550b`）
- 考試租戶隔離實機驗收找到三個回歸（`42e101b4`），最嚴重那個是
  **客戶一開始用，內部同事的作戰台就被客戶的場次蓋掉**
- 版面對齊 GodUI／AskMe（`dfb2b507`）

**使用者明說不做的**：第一家客戶、合約。不要再提醒。

**留下來的懸空引用**（刻意不改）：六個原始碼檔的註解仍以「產品化規格 N §X」引用已刪的文件
（docker-env.js／testenv-db-role.js／platform-backup.js／health-check-runner.js 等）。
那些註解講的是「為什麼這樣做」，本身仍然成立。

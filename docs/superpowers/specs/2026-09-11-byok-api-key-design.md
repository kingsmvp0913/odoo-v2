...
日期：2026-09-11
狀態：P1～P4 已於 2026-09-14 裁決；認證切換已上線，單張任務花費上限與公司管理員用量報表已完成程式實作、待上線驗證；預設金額仍待裁決（進度以「開發順序」§0 為準）
...
2026-09-23 本地資料庫 67 張有用量的任務，依現行估算式得到 p90 約 US$5.81、p99 約 US$28.20，最高約 US$45.94。這是候選數值，不包含沒有產生 `result` 的失敗輪；正式預設仍為 `NULL`（不啟用），待產品負責人裁決後才設定。程式實作在每次容器 AI 呼叫前重算餘額；若有上限而完整容器模式未啟用，任務直接停止，避免舊路徑繞過限制。

2026-09-23 續查：格式補救、部署失敗分類、退回分類與任務衍生的 Wiki 漂移分類也須帶原任務的 DB ID，才能吃到同一筆剩餘額度。背景分類找不到原任務時不開 AI；人工退回的分類以任務建立者（非按退回的人）認證與記帳。
...
用量列會記下執行當下的 `company_id`，公司報表只讀該快照，不依帳號目前所屬公司重算。上線前沒有公司快照的舊用量列不回填到客戶報表，避免帳號移轉後把別家歷史費用誤歸給新公司；平台管理員原有全平台報表仍可查舊資料。

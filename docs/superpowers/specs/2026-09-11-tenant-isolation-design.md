...
日期：2026-09-11
狀態：初稿，待使用者審閱（含「待你決定」§8）。**09-14 依使用者裁決改版**：角色改三種、內部人員＝「內部公司」成員、專案可掛多家公司
...
| 以下工具**改成只有平台管理員**：DB 查詢頁、DB 連線管理（含 SSH／VPN）、AI 執行畫面、專案／repo 管理、測試環境管理、上正式合併 main、流程圖／架構圖、考試 | 09-11 |
...
- 共用專案（例：鴻久與鴻伍綁同一個專案）時，兩家互相看得到該專案的任務、對話、Wiki——這是綁定本身的意義，綁之前要跟兩家確認。
...
**⚠ 共用專案的代價（§4.3 已載明，這裡重申因為它在開客戶當下才會被感覺到）：** 一個專案同時綁內部與客戶公司時，**兩邊互相看得到該專案的任務、對話、Wiki**。也就是說客戶會看到我們在他專案上開的內部任務。這是綁定本身的意義，不是缺陷；但如果有「不希望客戶看到的內部單」，那是另一個尚未解決的問題，開客戶之前要先決定怎麼處理（目前規格沒有「任務層級對客戶隱藏」的機制）。
...
| `loadTaskForActor`（改） | 現行「本人或 admin」再加上：任務所屬專案必須 `canSeeProject`。公司管理員能不能看同公司別人的任務，見 §8 P1 |
...
| `exam-routes.js`、`exam-upload-routes.js` | ~~平台管理員限定~~ → **公司設定的功能開關（客戶預設關閉）**<br>2026-09-21 使用者裁決推翻原本的「平台管理員限定」：考試系統本來就是給內部一般使用者考的，鎖成管理員限定等於把它從那 7 個人手上收走。改成由公司設定授予功能，隨第 3 部公司管理頁一起做。<br>⚠ 現況 24 支端點只掛 `verifyToken`，**這是建立第一家客戶公司之前的阻擋條件**。 |
| `settings.js` | 個人 GIT 所有人可用；Odoo 帳密與同步設定給客戶看不看，見 §8 P2 |
...
- **工具權限**：平台管理員以外的人（含內部公司成員）呼叫 §2 那些工具 → 403。

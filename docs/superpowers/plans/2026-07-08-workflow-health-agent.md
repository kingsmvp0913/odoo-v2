...
- Tests: `app/server/tests/health-data.test.js`、`health-check-runner.test.js`、`admin-health-check-routes.test.js`。
...
- Test: `app/server/tests/health-check-runner.test.js`（本任務先建檔、只驗 loadAgent 能解析與 render）
...
- [ ] **Step 1: 建 `app/server/tests/health-check-runner.test.js`（先只放 agent 契約測試）**
...
```bash
git add .claude/agents/workflow-health.md app/server/tests/health-check-runner.test.js
...
- Test: `app/server/tests/health-check-runner.test.js`（沿用 Task 3 檔案，追加 case）
...
- [ ] **Step 1: 追加測試到 `tests/health-check-runner.test.js`**（先加 mock 到檔頭、再加 case）：

於檔案**最上方**（`require` 之前）加 mock：
...
```javascript
      const map = { cs: '#7c3aed', triage: '#6b7280', analysis: '#2563eb', coding: '#059669',
                    qa: '#d97706', merge: '#db2777', deploy_fix: '#dc2626', wiki: '#0891b2', chat: '#f59e0b',
                    reject_classify: '#0d9488', workflow_health: '#7e22ce' };
```
